  ...
  class Slot
    ...
    def finish!
      raise Gameplay::Errors::FinishedSession,
        "Game is finished already" if session.finished? || session.lost? || session.won?
    ...
    def top_up!(top_up_amount = 100)
      raise Gameplay::Errors::FinishedSession,
        "You can't top up lost or finished sessions" if session.finished? || session.lost? || session.won?
